...
- Detailed highlight card (`HighlightCard.tsx`) supporting tag editing (`ManageTagsSheet.tsx`), favoriting, and deletion via `manage-highlight`.

---

## 4. Collections & Tagging (`/app/(app)/(tabs)/collections.tsx`)
- Organize highlights into custom tags and subject collections.
- Enables cross-book thematic research and note organization.

---

## 5. Semantic Search (`/app/(app)/(tabs)/search.tsx`)
...
## 6. Import & Clipping Ingestion (`/app/(app)/(tabs)/upload.tsx`)
...
## 7. Account & Settings (`/app/(app)/settings.tsx` & `profile.tsx`)
